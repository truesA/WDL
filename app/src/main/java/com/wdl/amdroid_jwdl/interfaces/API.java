package com.wdl.amdroid_jwdl.interfaces;

/**
 * Created by 62682 on 2018/3/20.
 */

public class API
{
    public static final String BASE_URL = "http://wdl.blackholebigdata.com/";
    public static final String User_Login ="login";
    public static final String Main_Data = "sa_kpi";
    public static final String Main_USERMSG = "user_search_detail";
    public static final String Main_USERMSG_BAST ="user_search_detail2";
    public static final String Main_mine = "bk_call";
    public static final String Main_msg = "user_rank";
    public static final String Msg_search = "user_search";
    public static final String Submit_Content = "submit_basic";
    public static final String Submit_Collection = "collection";
    public static final String Client_identity="user_search_detail/identity";
    public static final String Client_behavior="user_search_detail/behavior";
    public static final String Client_loyalty="user_search_detail/loyalty";
    public static final String Client_value="user_search_detail/value";
    public static final String Client_sticky="user_search_detail/sticky";
    //https://de70ebd3.ngrok.io/submit_identity?user_id=1&name=&plate_number=&gender=&car_model=&phone_number=&insurance_date=&sale_date=&car_age=8年
    public static final String Client_submit_identity="submit_identity";
     //http://wdl.blackholebigdata.com/bk_call?date=2018-05-26
    public static final String Main_saving = "bk_call";


}