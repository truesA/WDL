...
import com.wdl.jwdl.interfaces.DataService;
import com.wdl.jwdl.model.MainDataBean;
import com.wdl.jwdl.model.UserInfo;
import com.wdl.jwdl.util.PreferencesUtil;
import com.wdl.jwdl.util.UIUtils;
import com.wdl.jwdl.view.MyMarkerView;
...
//    public TextView taiC_m2;
    ...
    @BindView(R.id.sa_selsect)
    public TextView sa_selsects;
...
//        localYAxis.setDrawLimitLinesBehindData(true);
    ...
    private void setCombindData(List<Integer> barChartYsup, List<Integer> barChartYsdown, List<Double> line1, List<Double> line2) {
        ...
        List<Double> line3=new ArrayList<>();
        for (int i=0;i<32;i++){
            line3.add(0.0);
        }
        List<List<Double>> yLineDatat = new ArrayList<>();
        yLineDatat.add(line3);
    ...
    private void dealmeduler(MainDataBean resultBean, int checkedIds) {
        ...
        if (checkedIds == 0) {
            ...
            chanZ_today.setText(resultBean.getResult().getWeekday_mn_now().get(0).intValue() + "K");
            chanZ_sunday.setText(resultBean.getResult().getWeekend_mn_now().get(0).intValue() + "K");
        } else if (checkedIds == 1) { //台次
            sethtmlValue(taiC_mubv, "达成率", resultBean.getResult().getGoal_times_achv_rate().get(0) + "%");
...
//    }
     ...
     * @return
     */
    private LineData getLineData(List<Integer> lineChartY,  int lineColor) {
